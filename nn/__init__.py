...
class Ops:
    ...
    def _get_lib(cls):
        ...
        if cls._lib is None:
            # 加载C库 - 修改为正确的库名称
            cls._lib = ctypes.CDLL('./libtensor_ops.so')  # 改为 libtensor_ops.so
            ...
            cls._lib.reshape_forward.argtypes = [ctypes.POINTER(CTensor), ctypes.POINTER(CTensor)]

            # 初始化余弦查找表
            if hasattr(cls._lib, 'init_cos_lut'):
