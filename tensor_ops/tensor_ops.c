...
Tensor* create_tensor(int* shape, int ndim, DataType dtype) {
    ...
    switch (dtype) {
        ...
        case DTYPE_INT64:
            ...
            break;
        default:
            elem_size = 4;  // 默认float32
    }

    // 分配数据内存并初始化为0
    tensor->data = calloc(tensor->size, elem_size);  // 使用calloc初始化为0
 ...
 * @param output 输出张量
 */
void add_forward(const Tensor* input1, const Tensor* input2, Tensor* output) {
    // 简单的逐元素加法实现（假设形状已经广播）
    for (size_t i = 0; i < output->size; i++) {
        switch (output->dtype) {
            case DTYPE_FLOAT32: {
                float val1 = ((float*)input1->data)[i % input1->size];
                float val2 = ((float*)input2->data)[i % input2->size];
                ((float*)output->data)[i] = val1 + val2;
                break;
            }
            case DTYPE_FLOAT64: {
                double val1 = ((double*)input1->data)[i % input1->size];
                double val2 = ((double*)input2->data)[i % input2->size];
                ((double*)output->data)[i] = val1 + val2;
                break;
            }
            case DTYPE_FLOAT16: {
                uint16_t val1 = ((uint16_t*)input1->data)[i % input1->size];
                uint16_t val2 = ((uint16_t*)input2->data)[i % input2->size];
                float fval1 = float16_to_float(val1);
                float fval2 = float16_to_float(val2);
                float result = fval1 + fval2;
                ((uint16_t*)output->data)[i] = float_to_float16(result);
                break;
            }
            case DTYPE_BFLOAT16: {
                uint16_t val1 = ((uint16_t*)input1->data)[i % input1->size];
                uint16_t val2 = ((uint16_t*)input2->data)[i % input2->size];
                float fval1 = bfloat16_to_float(val1);
                float fval2 = bfloat16_to_float(val2);
                float result = fval1 + fval2;
                ((uint16_t*)output->data)[i] = float_to_bfloat16(result);
                break;
            }
            case DTYPE_INT8: {
                int8_t val1 = ((int8_t*)input1->data)[i % input1->size];
                int8_t val2 = ((int8_t*)input2->data)[i % input2->size];
                ((int8_t*)output->data)[i] = val1 + val2;
                break;
            }
            case DTYPE_INT16: {
                int16_t val1 = ((int16_t*)input1->data)[i % input1->size];
                int16_t val2 = ((int16_t*)input2->data)[i % input2->size];
                ((int16_t*)output->data)[i] = val1 + val2;
                break;
            }
            case DTYPE_INT32: {
                int32_t val1 = ((int32_t*)input1->data)[i % input1->size];
                int32_t val2 = ((int32_t*)input2->data)[i % input2->size];
                ((int32_t*)output->data)[i] = val1 + val2;
                break;
            }
            case DTYPE_INT64: {
                int64_t val1 = ((int64_t*)input1->data)[i % input1->size];
                int64_t val2 = ((int64_t*)input2->data)[i % input2->size];
                ((int64_t*)output->data)[i] = val1 + val2;
                break;
            }
        }
    }
 ...
 * @param output 输出张量
 */
void mul_forward(const Tensor* input1, const Tensor* input2, Tensor* output) {
    for (size_t i = 0; i < output->size; i++) {
        switch (output->dtype) {
            case DTYPE_FLOAT32: {
                float val1 = ((float*)input1->data)[i % input1->size];
 ...
 * @param output 输出张量
 */
void sub_forward(const Tensor* input1, const Tensor* input2, Tensor* output) {
    for (size_t i = 0; i < output->size; i++) {
        switch (output->dtype) {
            case DTYPE_FLOAT32: {
                float val1 = ((float*)input1->data)[i % input1->size];
 ...
 * @param output 输出张量
 */
void div_forward(const Tensor* input1, const Tensor* input2, Tensor* output) {
    for (size_t i = 0; i < output->size; i++) {
        switch (output->dtype) {
            case DTYPE_FLOAT32: {
                float val1 = ((float*)input1->data)[i % input1->size];
                float val2 = ((float*)input2->data)[i % input2->size];
                // 避免除零错误
                ((float*)output->data)[i] = val2 != 0.0f ? val1 / val2 : 0.0f;
                break;
            }
            case DTYPE_FLOAT64: {
                double val1 = ((double*)input1->data)[i % input1->size];
                double val2 = ((double*)input2->data)[i % input2->size];
                ((double*)output->data)[i] = val2 != 0.0 ? val1 / val2 : 0.0;
                break;
            }
            case DTYPE_FLOAT16: {
                uint16_t val1 = ((uint16_t*)input1->data)[i % input1->size];
                ...
                float fval1 = float16_to_float(val1);
                float fval2 = float16_to_float(val2);
                float result = fval2 != 0.0f ? fval1 / fval2 : 0.0f;
                ((uint16_t*)output->data)[i] = float_to_float16(result);
                break;
            }
            case DTYPE_BFLOAT16: {
            ...
            case DTYPE_INT8: {
                int8_t val1 = ((int8_t*)input1->data)[i % input1->size];
                int8_t val2 = ((int8_t*)input2->data)[i % input2->size];
                ((int8_t*)output->data)[i] = val2 != 0 ? val1 / val2 : 0;
                break;
            }
            case DTYPE_INT16: {
                int16_t val1 = ((int16_t*)input1->data)[i % input1->size];
                int16_t val2 = ((int16_t*)input2->data)[i % input2->size];
                ((int16_t*)output->data)[i] = val2 != 0 ? val1 / val2 : 0;
                break;
            }
            case DTYPE_INT32: {
                int32_t val1 = ((int32_t*)input1->data)[i % input1->size];
                int32_t val2 = ((int32_t*)input2->data)[i % input2->size];
                ((int32_t*)output->data)[i] = val2 != 0 ? val1 / val2 : 0;
                break;
            }
            case DTYPE_INT64: {
                int64_t val1 = ((int64_t*)input1->data)[i % input1->size];
                int64_t val2 = ((int64_t*)input2->data)[i % input2->size];
                ((int64_t*)output->data)[i] = val2 != 0 ? val1 / val2 : 0;
                break;
            }
        }
    }
...
void reshape_forward(const Tensor* input, Tensor* output) {
    // 简单的数据复制（假设总元素数相同）
    size_t copy_size = input->size < output->size ? input->size : output->size;
    ...
    switch (input->dtype) {
        case DTYPE_FLOAT32:
            ...
            break;
        case DTYPE_FLOAT64:
            memcpy(output->data, input->data, copy_size * sizeof(double));
            break;
        case DTYPE_FLOAT16:
        case DTYPE_BFLOAT16:
            memcpy(output->data, input->data, copy_size * sizeof(uint16_t));
            break;
        case DTYPE_INT8:
            memcpy(output->data, input->data, copy_size * sizeof(int8_t));
            break;
        case DTYPE_INT16:
            memcpy(output->data, input->data, copy_size * sizeof(int16_t));
            break;
        case DTYPE_INT32:
            memcpy(output->data, input->data, copy_size * sizeof(int32_t));
            break;
        case DTYPE_INT64:
            memcpy(output->data, input->data, copy_size * sizeof(int64_t));
            break;
    }
}
