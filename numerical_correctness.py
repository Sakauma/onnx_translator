"""文件功能：生成随机输入，调用 C 后端与 CUDA 参考程序，并比较算子数值正确性。
作者：Egor Izmaylov
时间：2026-06-02
"""
...
# 实现 `get_dtype_limits` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `float32_to_bfloat16_bits` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `bfloat16_bits_to_float32` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `decode_float8_e4m3` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `decode_float8_e5m2` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `to_float32` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `generate_random_data` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `run_cuda_ground_truth` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `random_uniform_like_reference` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `check_accuracy` 步骤，规范化输入并返回下游期望的数据或元信息。
...
# 实现 `verify_op` 步骤，规范化输入并返回下游期望的数据或元信息。
