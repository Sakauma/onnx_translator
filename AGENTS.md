...
本项目包含 `tests/` pytest 回归测试，同时保留脚本级验证。修改算子后，依次运行 `make`、`python -m pytest -q tests`，并在需要 CUDA 数值对比时运行 `bash compile_cuda.sh` 和 `python numerical_correctness.py`。修改导入器或图逻辑后，运行 `python graph_logic.py`、`python verify_graph.py` 或 `python tools/verify_all.py --skip-cuda`。新增算子测试应写入对应 `tests/test_operator_*.py` 文件，并在 `tools/numerical/cli.py` 中补充数值计划，覆盖形状推断、广播、类型提升，以及相关数值边界情况。
