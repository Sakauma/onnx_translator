/**
  ******************************************************************************
  * @file        verify_leaky_relu.cu
  * @author      Egor Izmaylov
  * @brief       提供 leaky_relu 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// LeakyRelu 按 ONNX 定义逐元素计算 x >= 0 ? x : alpha * x。
__global__ void leaky_relu_kernel(const float* input, float* output, size_t n, float alpha) {
...
int main(int argc, char** argv) {
    ...
    leaky_relu_kernel<<<blocks, threads>>>(d_input, d_output, n, alpha);
