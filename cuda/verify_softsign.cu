/**
  ******************************************************************************
  * @file        verify_softsign.cu
  * @author      Egor Izmaylov
  * @brief       提供 softsign 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// Softsign 按 ONNX 定义逐元素计算 x / (1 + abs(x))。
__global__ void softsign_kernel(const float* input, float* output, size_t n) {
    size_t t = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (t < n) {
        float x = input[t];
        output[t] = x / (1.0f + fabsf(x));
    }
}
...
int main(int argc, char** argv) {
    ...
    softsign_kernel<<<blocks, threads>>>(d_input, d_output, n);
