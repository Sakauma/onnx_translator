/**
  ******************************************************************************
  * @file        verify_softplus.cu
  * @author      Egor Izmaylov
  * @brief       提供 softplus 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// Softplus 按 ONNX 定义逐元素计算 log(1 + exp(x))。
__global__ void softplus_kernel(const float* input, float* output, size_t n) {
    size_t t = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (t < n) {
        float x = input[t];
        output[t] = log1pf(expf(x));
    }
}
...
int main(int argc, char** argv) {
    ...
    softplus_kernel<<<blocks, threads>>>(d_input, d_output, n);
