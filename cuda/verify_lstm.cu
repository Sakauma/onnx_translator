...
// 按 layout 读取和写入 initial_h/initial_c/Y_h/Y_c；内部状态保持 directions-first。
__device__ size_t lstm_state_index(int layout, int num_dirs, int batch, int hidden, int d, int b, int h) {
    return layout == 1
...
// 实现 LSTM 主输出 `Y`、最终隐藏状态 `Y_h` 和最终 cell 状态 `Y_c` 的 CUDA 参考 kernel，覆盖 activation、clip、peephole 和 input_forget 语义。
__global__ void lstm_kernel(
    ...
    for (int d = 0; d < num_dirs; ++d) {
        for (int b = 0; b < batch; ++b) {
            for (int h = 0; h < hidden; ++h) {
                ...
                size_t input_idx = lstm_state_index(layout, num_dirs, batch, hidden, d, b, h);
                h_state[idx] = initial_h[input_idx];
                c_state[idx] = initial_c[input_idx];
            }
        }
    }
    ...
    for (int d = 0; d < num_dirs; ++d) {
        ...
        int f_code = recurrent_activation_code(runtime, d * 3, 1);
        int g_code = recurrent_activation_code(runtime, d * 3 + 1, 0);
        int h_code = recurrent_activation_code(runtime, d * 3 + 2, 0);
        for (int step = 0; step < seq_len; ++step) {
            ...
            for (int b = 0; b < batch; ++b) {
                for (int h = 0; h < hidden; ++h) {
                    ...
                    double i_gate = recurrent_activation_value(recurrent_clip_value(gates[0] + p_i * c_prev, runtime), f_code, runtime, d * 3);
                    double f_gate = input_forget ? (1.0 - i_gate) : recurrent_activation_value(recurrent_clip_value(gates[2] + p_f * c_prev, runtime), f_code, runtime, d * 3);
                    double c_bar = recurrent_activation_value(recurrent_clip_value(gates[3], runtime), g_code, runtime, d * 3 + 1);
                    double c_val = f_gate * c_prev + i_gate * c_bar;
                    double o_gate = recurrent_activation_value(recurrent_clip_value(gates[1] + p_o * c_val, runtime), f_code, runtime, d * 3);

                    h_next[(size_t)b * hidden + h] = o_gate * recurrent_activation_value(c_val, h_code, runtime, d * 3 + 2);
    ...
    for (int d = 0; d < num_dirs; ++d) {
        for (int b = 0; b < batch; ++b) {
            for (int h = 0; h < hidden; ++h) {
                size_t idx = ((size_t)d * batch + b) * hidden + h;
                size_t output_idx = lstm_state_index(layout, num_dirs, batch, hidden, d, b, h);
                Y_h[output_idx] = h_state[idx];
                Y_c[output_idx] = c_state[idx];
            }
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    FILE* f = fopen(argv[2], "rb"); verify_fread_exact(h_x, sizeof(double), x_len, f); verify_close_file(f);
    f = fopen(argv[3], "rb"); verify_fread_exact(h_w, sizeof(double), w_len, f); verify_close_file(f);
    f = fopen(argv[4], "rb"); verify_fread_exact(h_r, sizeof(double), r_len, f); verify_close_file(f);
    f = fopen(argv[5], "rb"); verify_fread_exact(h_b, sizeof(double), b_len, f); verify_close_file(f);
    f = fopen(argv[6], "rb"); verify_fread_exact(h_seq, sizeof(int64_t), (size_t)batch, f); verify_close_file(f);
    f = fopen(argv[7], "rb"); verify_fread_exact(h_init_h, sizeof(double), h_len, f); verify_close_file(f);
    f = fopen(argv[8], "rb"); verify_fread_exact(h_init_c, sizeof(double), h_len, f); verify_close_file(f);
    f = fopen(argv[9], "rb"); verify_fread_exact(h_p, sizeof(double), p_len, f); verify_close_file(f);

    double *d_x, *d_w, *d_r, *d_b, *d_init_h, *d_init_c, *d_p, *d_y, *d_yh, *d_yc, *d_workspace;
    int64_t* d_seq;
    CUDA_CHECK(cudaMalloc(&d_x, x_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_w, w_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_r, r_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_b, b_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_seq, (size_t)batch * sizeof(int64_t)));
    CUDA_CHECK(cudaMalloc(&d_init_h, h_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_init_c, h_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_p, p_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_y, out_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_yh, h_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_yc, h_len * sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_workspace, (2 * h_len + 2 * (size_t)batch * hidden) * sizeof(double)));
    CUDA_CHECK(cudaMemcpy(d_x, h_x, x_len * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_w, h_w, w_len * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_r, h_r, r_len * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_b, h_b, b_len * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_seq, h_seq, (size_t)batch * sizeof(int64_t), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_init_h, h_init_h, h_len * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_init_c, h_init_c, h_len * sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_p, h_p, p_len * sizeof(double), cudaMemcpyHostToDevice));

    lstm_kernel<<<1, 1>>>(d_x, d_w, d_r, d_b, d_seq, d_init_h, d_init_c, d_p, d_y, d_yh, d_yc, d_workspace, seq_len, batch, input_size, num_dirs, hidden, direction, layout, input_forget, runtime);
    CUDA_CHECK_LAUNCH();
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaMemcpy(h_y, d_y, out_len * sizeof(double), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_yh, d_yh, h_len * sizeof(double), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_yc, d_yc, h_len * sizeof(double), cudaMemcpyDeviceToHost));
    ...
    if (!fo) return 7;
    verify_fwrite_exact(h_yh, sizeof(double), h_len, fo);
    verify_close_file(fo);
    fo = fopen("tmp_lstm_y_c.bin", "wb");
    if (!fo) return 8;
    verify_fwrite_exact(h_yc, sizeof(double), h_len, fo);
    verify_close_file(fo);
