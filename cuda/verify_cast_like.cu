/**
  ******************************************************************************
  * @file        verify_cast_like.cu
  * @author      Egor Izmaylov
  * @brief       提供 cast_like 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// CastLike 的输出 dtype 来自 target tensor；验证程序只需要读取 data 输入并按 runner 传入的 dtype 写回。
__global__ void cast_like_kernel(const float* input, void* output, size_t n, int output_kind) {
...
// 读取输出类型参数；target 输入文件只用于保持统一调用签名。
...
int main(int argc, char** argv) {
    // <out_len> <input.bin> <target.bin> <params.bin> <out.bin>
    if (argc != 6) {
        fprintf(stderr, "Usage: %s <out_len> <input.bin> <target.bin> <params.bin> <out.bin>\n", argv[0]);
        return 1;
    }
