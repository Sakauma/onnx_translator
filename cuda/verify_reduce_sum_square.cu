/**
  ******************************************************************************
  * @file        verify_reduce_sum_square.cu
  * @author      Egor Izmaylov
  * @brief       提供 reduce_sum_square 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// 对输入所有元素执行 sum(x^2)。
__global__ void reduce_sum_square_kernel(const float* in, float* out, int64_t n) {
...
int main(int argc, char** argv) {
    ...
    reduce_sum_square_kernel<<<1, 1>>>(d_in, d_out, p.in_len);
