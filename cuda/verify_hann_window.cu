/**
  ******************************************************************************
  * @file        verify_hann_window.cu
  * @author      Egor Izmaylov
  * @brief       提供 HannWindow 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
__global__ void hann_window_kernel(float* output, int32_t size, int32_t periodic) {
    ...
    output[i] = (float)(0.5 - 0.5 * cos(angle));
}
...
int main(int argc, char** argv) {
    ...
    hann_window_kernel<<<blocks, threads>>>(d_output, size, periodic);
