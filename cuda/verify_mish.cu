/**
  ...
  * @brief       提供 mish 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// Mish 按 ONNX 定义逐元素计算 x * tanh(softplus(x))。
__global__ void mish_kernel(const float* input, float* output, size_t n) {
    size_t t = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (t < n) {
        float x = input[t];
        float softplus = (x > 20.0f) ? x : log1pf(expf(x));
        output[t] = x * tanhf(softplus);
    }
}
