...
// Egor Izmaylov: Function `sub_kernel` is a CUDA reference kernel for the verifier; it maps thread indices to tensor elements and computes the expected GPU result.
__global__ void sub_kernel(const float* a, const float* b, float* out, size_t n) {
