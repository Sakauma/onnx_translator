/**
  ******************************************************************************
  * @file        verify_conv2d.cu
  * @author      Egor Izmaylov
  * @brief       提供 conv2d 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// 实现 `get_val_4d` 的 CUDA 验证辅助逻辑，为参考计算准备参数或中间结果。
...
// 实现 `conv2d_kernel` CUDA 参考 kernel，将线程索引映射到张量元素并计算期望输出。
...
int main(int argc, char** argv) {
    ...
    if(!fp) return 2;
    verify_fread_exact(p, sizeof(int), 16, fp);
