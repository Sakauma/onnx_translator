/**
  ...
  * @brief       提供 celu 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// Celu 按 ONNX 定义逐元素计算 max(0,x) + min(0, alpha * (exp(x / alpha) - 1))。
__global__ void celu_kernel(const float* input, float* output, size_t n, float alpha) {
    size_t t = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (t < n) {
        float x = input[t];
        output[t] = (x >= 0.0f) ? x : alpha * (expf(x / alpha) - 1.0f);
    }
}
...
int main(int argc, char** argv) {
    ...
    celu_kernel<<<blocks, threads>>>(d_input, d_output, n, alpha);
