...
__global__ void div_kernel_float32(const float* input_a, const float* input_b, float* output, size_t size) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
        if (input_b[idx] == 0.0f) output[idx] = 0.0f; 
        else output[idx] = input_a[idx] / input_b[idx];
    }
}
...
int main(int argc, char** argv) {
    ...
    div_kernel_float32<<<(num_elements + 255) / 256, 256>>>(d_a, d_b, d_out, num_elements);
    ...
    free(h_a); free(h_b); free(h_res); cudaFree(d_a); cudaFree(d_b); cudaFree(d_out);
    return 0;
}