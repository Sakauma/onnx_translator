...
// Egor Izmaylov: Function `div_kernel` is a CUDA reference kernel for the verifier; it maps thread indices to tensor elements and computes the expected GPU result.
__global__ void div_kernel(const float* a, const float* b, float* out, size_t n) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        //out[idx] = (b[idx] == 0.0f) ? 0.0f : (a[idx] / b[idx]);
        out[idx] = a[idx] / b[idx];
    }
}
...
int main(int argc, char** argv) {
    ...
    div_kernel<<<(n + 255)/256, 256>>>(d_a, d_b, d_out, n);
