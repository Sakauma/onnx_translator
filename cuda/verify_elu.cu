/**
  ...
  * @brief       提供 elu 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// Elu 按 ONNX 定义逐元素计算 x > 0 ? x : alpha * (exp(x) - 1)。
__global__ void elu_kernel(const float* input, float* output, size_t n, float alpha) {
    size_t t = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (t < n) {
        float x = input[t];
        output[t] = (x > 0.0f) ? x : alpha * (expf(x) - 1.0f);
    }
}
...
int main(int argc, char** argv) {
    ...
    elu_kernel<<<blocks, threads>>>(d_input, d_output, n, alpha);
