/**
  ...
  * @brief       提供 tile 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// 根据输出坐标对输入维度取模，得到 Tile 官方语义下的源元素坐标。
__global__ void tile_kernel(
    ...
    for (int d = 0; d < rank; ++d) {
        in_coords[d] = out_coords[d] % input_shape[d];
    }
...
int main(int argc, char** argv) {
    // <out_len> <input.bin> <repeats.bin> <params.bin> <out.bin>
    if (argc != 6) {
        printf("Usage: %s <out_len> <input.bin> <repeats.bin> <params.bin> <out.bin>\n", argv[0]);
        return 1;
    }
    ...
    for (int d = 0; d < rank; ++d) {
        if (input_shape[d] <= 0 || repeats[d] < 0) {
            printf("invalid shape or repeats\n");
            return 1;
        }
        output_shape[d] = input_shape[d] * repeats[d];
        in_len *= (size_t)input_shape[d];
        expected_out_len *= (size_t)output_shape[d];
    }
    ...
    tile_kernel<<<blocks, threads>>>(d_input, d_output, d_input_shape, d_output_shape, rank, out_len);
