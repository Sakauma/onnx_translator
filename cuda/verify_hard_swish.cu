/**
  ******************************************************************************
  * @file        verify_hard_swish.cu
  * @author      Egor Izmaylov
  * @brief       提供 hard_swish 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// HardSwish 按 ONNX 定义逐元素计算 x * max(0, min(1, x / 6 + 0.5))。
__global__ void hard_swish_kernel(const float* input, float* output, size_t n) {
    size_t t = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (t < n) {
        float x = input[t];
        output[t] = x * fmaxf(0.0f, fminf(1.0f, x / 6.0f + 0.5f));
    }
}
...
int main(int argc, char** argv) {
    ...
    hard_swish_kernel<<<blocks, threads>>>(d_input, d_output, n);
