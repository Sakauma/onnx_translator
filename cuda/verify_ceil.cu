/**
  ...
  * @brief       提供 ceil 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// Ceil 对输入逐元素向正无穷取整。
__global__ void ceil_kernel(const float* input, float* output, size_t n) {
    size_t t = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (t < n) {
        output[t] = ceilf(input[t]);
    }
}
...
int main(int argc, char** argv) {
    ...
    ceil_kernel<<<blocks, threads>>>(d_input, d_output, n);
