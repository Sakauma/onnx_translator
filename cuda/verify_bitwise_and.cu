/**
  ******************************************************************************
  * @file        verify_bitwise_and.cu
  * @author      Egor Izmaylov
  * @brief       提供 BitwiseAnd 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// 按 int32 位模式执行逐元素按位与。
__global__ void bitwise_and_kernel(const int32_t* lhs, const int32_t* rhs, int32_t* output, size_t out_len) {
...
int main(int argc, char** argv) {
    ...
    bitwise_and_kernel<<<blocks, threads>>>(d_lhs, d_rhs, d_output, out_len);
    CUDA_CHECK_LAUNCH();
