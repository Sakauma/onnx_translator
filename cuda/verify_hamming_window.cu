/**
  ******************************************************************************
  * @file        verify_hamming_window.cu
  * @author      Egor Izmaylov
  * @brief       提供 HammingWindow 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
__global__ void hamming_window_kernel(float* output, int32_t size, int32_t periodic) {
    ...
    output[i] = (float)((25.0 / 46.0) - (21.0 / 46.0) * cos(angle));
}
