/**
  ...
  * @brief       提供 mean 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// Mean 对多个已经按 ONNX 广播规则展开的输入逐元素求平均。
__global__ void mean_kernel(const float* inputs, float* output, size_t n, int num_inputs) {
    ...
    output[t] = (float)(acc / (double)num_inputs);
}
...
int main(int argc, char** argv) {
    ...
    float* d_inputs = NULL;
    float* d_output = NULL;
    CUDA_CHECK(cudaMalloc(&d_inputs, all_bytes));
    CUDA_CHECK(cudaMalloc(&d_output, bytes));
    CUDA_CHECK(cudaMemcpy(d_inputs, h_inputs, all_bytes, cudaMemcpyHostToDevice));

    int threads = 256;
    int blocks = (int)((n + (size_t)threads - 1) / (size_t)threads);
    mean_kernel<<<blocks, threads>>>(d_inputs, d_output, n, num_inputs);
    CUDA_CHECK_LAUNCH();
