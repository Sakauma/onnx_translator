/**
  ******************************************************************************
  * @file        verify_space_to_depth.cu
  * @author      Egor Izmaylov
  * @brief       提供 SpaceToDepth 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// 保存 SpaceToDepth 参考计算所需的 NCHW 输入形状和 blocksize。
...
// 按输出坐标反推输入坐标，将空间 block 展开到通道维。
__global__ void space_to_depth_kernel(
    const float* input,
    float* output,
    SpaceToDepthParams p,
    size_t out_len
) {
    ...
    int in_c = c % p.channels;
    int rem = c / p.channels;
    int dy = rem / p.blocksize;
    int dx = rem % p.blocksize;
    int in_h = h * p.blocksize + dy;
    int in_w = w * p.blocksize + dx;
...
// 顺序读取 `[N, C, H, W, blocksize]` 参数。
static int read_space_to_depth_params(const char* params_path, SpaceToDepthParams* params) {
    ...
    int32_t values[5];
    if (fread(values, sizeof(int32_t), 5, fp) != 5) {
        fclose(fp);
    ...
    return params->batch > 0 && params->channels > 0 && params->height > 0
        ...
        && params->height % params->blocksize == 0
        && params->width % params->blocksize == 0;
}
...
int main(int argc, char** argv) {
    ...
    if (!read_space_to_depth_params(params_path, &params)) {
        return 1;
    }
    ...
    space_to_depth_kernel<<<blocks, threads>>>(d_input, d_output, params, out_len);
