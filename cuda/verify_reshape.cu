/**
  ******************************************************************************
  * @file        verify_reshape.cu
  * @author      Egor Izmaylov
  * @brief       提供 reshape 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// Reshape 只改变输出形状，不改变元素线性顺序；CUDA 参考按平坦顺序复制输入。
...
int main(int argc, char** argv) {
    // <out_len> <input.bin> <shape.bin> <out.bin>
    if (argc != 5) {
        printf("Usage: %s <out_len> <input.bin> <shape.bin> <out.bin>\n", argv[0]);
        return 1;
    }
