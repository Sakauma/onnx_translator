/**
  ******************************************************************************
  * @file        verify_swish.cu
  * @author      Egor Izmaylov
  * @brief       提供 swish 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
...
// Swish 按 ONNX 定义逐元素计算 x * sigmoid(alpha * x)。
__global__ void swish_kernel(const float* input, float* output, size_t n, float alpha) {
    size_t t = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (t < n) {
        float x = input[t];
        float z = alpha * x;
        float sigmoid = z >= 0.0f ? 1.0f / (1.0f + expf(-z)) : expf(z) / (1.0f + expf(z));
        output[t] = x * sigmoid;
    }
}
...
int main(int argc, char** argv) {
    ...
    swish_kernel<<<blocks, threads>>>(d_input, d_output, n, alpha);
