/**
  ******************************************************************************
  * @file        verify_bitwise_xor.cu
  * @author      Egor Izmaylov
  * @brief       提供 BitwiseXor 算子的 CUDA 参考验证程序，供数值正确性脚本与 C 后端结果对比。
  * @details     2026.06.05  V1.0.0  创建
  ******************************************************************************
  * @attention
  ******************************************************************************
*/

#include <cuda_runtime.h>
#include "verify_common.cuh"
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>

// 按 int32 位模式执行逐元素按位异或。
__global__ void bitwise_xor_kernel(const int32_t* lhs, const int32_t* rhs, int32_t* output, size_t out_len) {
    size_t tid = (size_t)blockIdx.x * (size_t)blockDim.x + (size_t)threadIdx.x;
    if (tid >= out_len) return;
    output[tid] = lhs[tid] ^ rhs[tid];
}

// 作为 CUDA 验证程序入口，从二进制文件读取输入、执行参考计算并写回结果。
int main(int argc, char** argv) {
    // <out_len> <lhs.bin> <rhs.bin> <out.bin>
    if (argc != 5) {
        fprintf(stderr, "Usage: %s <out_len> <lhs.bin> <rhs.bin> <out.bin>\n", argv[0]);
        return 1;
    }

    size_t out_len = (size_t)atoll(argv[1]);
    const char* lhs_path = argv[2];
    const char* rhs_path = argv[3];
    const char* out_path = argv[4];
    size_t bytes = out_len * sizeof(int32_t);

    int32_t* h_lhs = (int32_t*)malloc(bytes);
    int32_t* h_rhs = (int32_t*)malloc(bytes);
    int32_t* h_output = (int32_t*)malloc(bytes);
    if (!h_lhs || !h_rhs || !h_output) {
        fprintf(stderr, "malloc failed\n");
        return 1;
    }

    FILE* lhs_fp = fopen(lhs_path, "rb");
    FILE* rhs_fp = fopen(rhs_path, "rb");
    if (!lhs_fp || !rhs_fp) {
        fprintf(stderr, "open input failed\n");
        return 1;
    }
    size_t lhs_read = fread(h_lhs, sizeof(int32_t), out_len, lhs_fp);
    size_t rhs_read = fread(h_rhs, sizeof(int32_t), out_len, rhs_fp);
    verify_close_file(lhs_fp);
    verify_close_file(rhs_fp);
    if (lhs_read != out_len || rhs_read != out_len) {
        fprintf(stderr, "read input failed\n");
        return 1;
    }

    int32_t* d_lhs = NULL;
    int32_t* d_rhs = NULL;
    int32_t* d_output = NULL;
    CUDA_CHECK(cudaMalloc(&d_lhs, bytes));
    CUDA_CHECK(cudaMalloc(&d_rhs, bytes));
    CUDA_CHECK(cudaMalloc(&d_output, bytes));
    CUDA_CHECK(cudaMemcpy(d_lhs, h_lhs, bytes, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_rhs, h_rhs, bytes, cudaMemcpyHostToDevice));

    int threads = 256;
    int blocks = (int)((out_len + (size_t)threads - 1) / (size_t)threads);
    bitwise_xor_kernel<<<blocks, threads>>>(d_lhs, d_rhs, d_output, out_len);
    CUDA_CHECK_LAUNCH();
    CUDA_CHECK(cudaDeviceSynchronize());

    CUDA_CHECK(cudaMemcpy(h_output, d_output, bytes, cudaMemcpyDeviceToHost));

    FILE* out_fp = fopen(out_path, "wb");
    if (!out_fp) {
        fprintf(stderr, "open output failed\n");
        return 1;
    }
    size_t written = fwrite(h_output, sizeof(int32_t), out_len, out_fp);
    verify_close_file(out_fp);
    if (written != out_len) {
        fprintf(stderr, "write output failed\n");
        return 1;
    }

    CUDA_CHECK(cudaFree(d_lhs));
    CUDA_CHECK(cudaFree(d_rhs));
    CUDA_CHECK(cudaFree(d_output));
    free(h_lhs);
    free(h_rhs);
    free(h_output);
    return 0;
}
