...
- `python tools/cli.py compile-cuda`：通过，`167` 个 CUDA verifier 编译成功。
...
以下 `10` 个普通 C-backed 算子仍缺少独立 CUDA verifier 或默认 active numerical plan，后续应继续补 C/CUDA reference 和默认数值门禁，而不是回退为普通数值路径的 Python-only 实现。
