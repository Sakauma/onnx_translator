...
- `python tools/cli.py compile-cuda`：通过，`173` 个 CUDA verifier 编译成功。
...
以下 `4` 个普通 C-backed 算子仍缺少独立 CUDA verifier 或默认 active numerical plan，后续应继续补 C/CUDA reference 和默认数值门禁，而不是回退为普通数值路径的 Python-only 实现。
...
- `Multinomial` 需要先确定 deterministic seed、概率归一化和多次采样输出 dtype 的 CUDA reference 口径。
- 损失函数和 NMS 需要重点验证 ignore index、reduction、阈值比较、排序 tie-break 和空输出。

## 后续建议

1. 先处理 `NegativeLogLikelihoodLoss`、`SoftmaxCrossEntropyLoss` 和 `NonMaxSuppression`，每个算子先补官方 reference pytest 边界，再接 CUDA numerical。
2. 最后处理 `Multinomial`，覆盖 one-hot、零概率、非归一化概率和 int32/int64 输出路径。
