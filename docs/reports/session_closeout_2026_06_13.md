...
以下 `3` 个普通 C-backed 算子仍需要继续补充更多独立 CUDA/numerical 属性、dtype 和边界 case，后续应继续扩展 C/CUDA reference 和默认数值门禁，而不是回退为普通数值路径的 Python-only 实现。
...
1. 先处理 `NegativeLogLikelihoodLoss`、`SoftmaxCrossEntropyLoss` 和 `NonMaxSuppression`，每个算子继续补官方 reference pytest 边界和 CUDA numerical case。
2. 再按高风险属性矩阵扩展 ROI、序列、谱、Attention、DeformConv、量化和采样类算子的边界集合。
