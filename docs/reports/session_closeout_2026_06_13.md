...
- `python tools/cli.py compile-cuda`：通过，`169` 个 CUDA verifier 编译成功。
...
以下 `8` 个普通 C-backed 算子仍缺少独立 CUDA verifier 或默认 active numerical plan，后续应继续补 C/CUDA reference 和默认数值门禁，而不是回退为普通数值路径的 Python-only 实现。
...
- 随机采样算子需要先确定 deterministic seed、统计容差和分布验证口径。
...
2. 最后集中处理随机/采样算子，先制定 seed 和统计容差策略，再纳入默认门禁。
