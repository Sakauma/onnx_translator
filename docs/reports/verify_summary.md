...
- QuantizeLinear/DequantizeLinear 当前 C/CUDA numerical 已覆盖 scalar、`axis=1` uint8 per-axis scale/zero_point、`axis=-1` signed int8 per-axis 和省略 `zero_point` 默认零点；新版属性，例如 block quantization、output_dtype、precision、saturate 等，仍需结合目标 opset 再决定是否扩展。
