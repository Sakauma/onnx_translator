...
- QuantizeLinear/DequantizeLinear 当前 C/CUDA numerical 已覆盖 scalar、`axis=1` uint8 per-axis scale/zero_point、`axis=-1` signed int8 per-axis 和省略 `zero_point` 默认零点，并已补充 `output_dtype` 属性导入和省略零点主路径 pytest 覆盖；新版 block quantization、precision、saturate 等属性仍需结合目标 opset 再决定是否扩展。
