...
继续补强 `QuantizeLinear` 最新 schema 的 float8 `saturate` 属性：C 后端新增 `quantize_linear_forward_precision_saturate` 入口，并在 float8 输出写回时按 ONNX reference 的 E4M3FN/E5M2 溢出阈值区分最大有限值、Inf 与 NaN；旧 `quantize_linear_forward` 和 `quantize_linear_forward_precision` ABI 保持兼容。CUDA verifier 同步接收 saturate 参数，numerical runner 将 float8 目标 dtype 和 saturate 属性写入参数块。默认 numerical 新增 float8_e5m2 `saturate=1/0` 和 float8_e4m3 `saturate=0` 三条计划；pytest 使用 ONNX `ReferenceEvaluator` 的 raw uint8 位模式校验 C 后端输出。验证命令：`/home/sakauma/data/miniconda3/envs/egor/bin/python -m py_compile nn/__init__.py nn/operators/quantization.py tools/numerical/runner.py tools/numerical/cli.py tests/test_operator_core_numeric_semantics.py`、`make PYTHON=/home/sakauma/data/miniconda3/envs/egor/bin/python`、`/home/sakauma/data/miniconda3/envs/egor/bin/python -m pytest -q tests/test_operator_core_numeric_semantics.py -k 'quantize_linear_float8_saturate or precision_double'`、完整 `/home/sakauma/data/miniconda3/envs/egor/bin/python tools/cli.py compile-cuda`、`/home/sakauma/data/miniconda3/envs/egor/bin/python tools/cli.py numerical --op quantize_linear --iterations 3 --skip-plots` 和 `/home/sakauma/data/miniconda3/envs/egor/bin/python tools/audit_ops.py --output docs/reports/operator_coverage.md` 均已通过；targeted pytest 结果为 `4 passed, 17 deselected`，QuantizeLinear targeted numerical 提升到 `17` 组计划、`51` 个样本全部通过。覆盖报告已刷新，默认 active numerical plan 提升到 `705` 条计划，混合精度计划提升到 `470` 条。
