...
- QuantizeLinear/DequantizeLinear 当前 C/CUDA numerical 已覆盖 scalar、`axis=1` uint8 per-axis scale/zero_point、`axis=-1` signed int8 per-axis、省略 `zero_point` 默认零点、`output_dtype` 属性和 `block_size=2` blocked scale/zero_point；`precision`、`saturate`、更多 block 形状/轴组合以及 float4/float8/低比特量化 dtype 仍需继续扩展。
