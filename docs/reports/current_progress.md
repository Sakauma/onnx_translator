...
- CUDA verifier：`178` 个。
- 默认 active numerical plan：`178` 个唯一算子名称，`711` 条默认计划。
- 默认 active numerical plan 混合精度覆盖：`476` 条计划。
...
- `python tools/cli.py compile-cuda`
  - 最近记录结果：`178` 个 CUDA verifier 编译成功；`verify_dft` 已升级为按 rank、shape 和 axis 执行独立坐标映射。
- `python tools/cli.py numerical --op dft --op stft --iterations 3 --skip-plots`
  - 最近记录结果：DFT/STFT 各 `9` 个 active plan 样本均通过；新增 DFT 4D 中间轴 `axis=1`、`dft_length=5`，以及 STFT 4D 多前缀维输入。
- `python -m pytest -q tests/test_operator_spectral_semantics.py`
  - 最近记录结果：`4 passed`，新增独立公式验证 DFT 高维中间轴和 STFT 多前缀维切帧。
...
- `python -m pytest -q tests/test_operator_import_and_shape.py::test_quantize_linear_forward_shape_and_optional_zero_point_import tests/test_operator_core_numeric_semantics.py::test_c_backend_quantize_and_dequantize_block_size_match_onnx_reference tests/test_operator_core_numeric_semantics.py::test_c_backend_quantize_dequantize_output_dtype_without_zero_point_matches_onnx_reference`
  - 最近记录结果：`3 passed`；新增 `block_size=2` 的 blocked quantization 导入属性保留，以及输入 `(2, 3, 4)`、scale/zero_point `(2, 2, 4)`、`axis=1` 的 QuantizeLinear/DequantizeLinear ONNX reference 对齐测试。
- `python tools/cli.py numerical --op quantize_linear --op dequantize_linear --iterations 3 --skip-plots`
  - 最近记录结果：QuantizeLinear/DequantizeLinear 共 `18` 组计划、`54` 个样本全部通过；新增 float32、float16、bfloat16 分支中的 `block_size=2` blocked scale/zero_point 映射 case。
- `python -m pytest -q tests/test_operator_core_numeric_semantics.py::test_c_backend_quantize_and_dequantize_16bit_integer_dtypes_match_onnx_reference`
  - 最近记录结果：`1 passed`；新增 QuantizeLinear int16/uint16 饱和范围和 DequantizeLinear int16/uint16 官方 dtype 约束的 ONNX reference 对齐测试。
- `python tools/cli.py numerical --op quantize_linear --op dequantize_linear --iterations 3 --skip-plots`
  - 最近记录结果：QuantizeLinear/DequantizeLinear 共 `22` 组计划、`66` 个样本全部通过；新增 float32 uint16、float16 int16、float32 int16 反量化和 bfloat16 uint16 反量化路径。
...
- `python -m pytest -q tests/test_operator_core_numeric_semantics.py::test_c_backend_dequantize_int32_dtype_matches_onnx_reference`
  - 最近记录结果：`1 passed`；新增 DequantizeLinear 的 int32 输入和 int32 zero point 官方 dtype 约束 ONNX reference 对齐测试，覆盖 int32 极值和大整数反量化。
- `python tools/cli.py numerical --op quantize_linear --op dequantize_linear --iterations 3 --skip-plots`
...
- `python tools/cli.py numerical --op quantize_linear --iterations 3 --skip-plots`
  - 最近记录结果：QuantizeLinear 共 `17` 组计划、`51` 个样本全部通过；新增 float8_e5m2 `saturate=1/0` 溢出有限值/Inf 行为，以及 float8_e4m3 `saturate=0` 有限舍入与 NaN 溢出边界，均与独立 CUDA verifier 对齐。
- `python tools/cli.py numerical --op quantize_linear --op dequantize_linear --iterations 3 --skip-plots`
  - 最近记录结果：QuantizeLinear/DequantizeLinear 覆盖 float8_e4m3fnuz 与 float8_e5m2fnuz 的 `saturate=1/0`、FNUZ NaN 位模式和反量化解码边界，共 `63` 个 QuantizeLinear 样本与 `48` 个 DequantizeLinear 样本全部通过。
- `python tools/cli.py numerical --iterations 1 --skip-plots`
  - 最近记录结果：本轮新增 6 条 QuantizeLinear/DequantizeLinear FNUZ 默认计划后，当前默认门禁提升到 `711` 条；完整 numerical 一轮已全部通过，覆盖 `178` 个唯一算子和 `476` 条混合精度计划。
- `python -m pytest -q tests`
  - 最近记录结果：`315 passed, 1 skipped`。
...
- 当前默认 numerical 中已经包含 `476` 条混合精度计划，覆盖 float16、bfloat16、float8_e4m3/e5m2、float8_e4m3fnuz/e5m2fnuz 以及相关低精度存储路径。
