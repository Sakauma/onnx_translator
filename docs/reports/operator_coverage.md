...
- 独立 pytest 深度语义/混合精度覆盖：9 个；`RNN`, `LRN`, `MeanVarianceNormalization`, `DFT`, `GRU`, `STFT`, `MaxRoiPool`, `LSTM`, `RoiAlign`。
- ONNX reference pytest 语义/混合精度覆盖：45 个；`Elu`, `EyeLike`, `Expand`, `Flatten`, `Selu`, `LeakyRelu`, `Reshape`, `ConstantOfShape`, `ThresholdedRelu`, `HardSigmoid`, `Range`, `Celu`, `Shrink`, `Tile`, `Transpose`, `Softplus`, `Softsign`, `Pad`, `HardSwish`, `Acos`, `Asin`, `Identity`, `Round`, `Cosh`, `Erf`, `Squeeze`, `Size`, `Sinh`, `BatchNormalization`, `Asinh`, `Acosh`, `Atanh`, `OneHot`, `Split`, `Unsqueeze`, `Where`, `Trilu`, `InstanceNormalization`, `DepthToSpace`, `LayerNormalization`, `Concat`, `SpaceToDepth`, `ReverseSequence`, `Compress`, `LpNormalization`。
...
- 当前没有记录暂缓后端化算子。
- 已补充独立 pytest 深度语义/混合精度覆盖的算子：`RNN`, `LRN`, `MeanVarianceNormalization`, `DFT`, `GRU`, `STFT`, `MaxRoiPool`, `LSTM`, `RoiAlign`。
- 已补充 ONNX reference pytest 语义/混合精度覆盖的普通算子：`Elu`, `EyeLike`, `Expand`, `Flatten`, `Selu`, `LeakyRelu`, `Reshape`, `ConstantOfShape`, `ThresholdedRelu`, `HardSigmoid`, `Range`, `Celu`, `Shrink`, `Tile`, `Transpose`, `Softplus`, `Softsign`, `Pad`, `HardSwish`, `Acos`, `Asin`, `Identity`, `Round`, `Cosh`, `Erf`, `Squeeze`, `Size`, `Sinh`, `BatchNormalization`, `Asinh`, `Acosh`, `Atanh`, `OneHot`, `Split`, `Unsqueeze`, `Where`, `Trilu`, `InstanceNormalization`, `DepthToSpace`, `LayerNormalization`, `Concat`, `SpaceToDepth`, `ReverseSequence`, `Compress`, `LpNormalization`。
- 未发现仍需立即后端化的 Python-only 普通数值/张量算子。
- 默认数值门禁当前覆盖 68 个唯一算子；尚有 64 个已实现算子未进入 active numerical plan。
...
| 11 | `EyeLike` | yes | yes | yes | `eye_like_forward` | `eye_like_forward` | no | no | 已 pytest 语义验证 | 含 Python 调度或 fallback; 已有 ONNX reference pytest 语义/混合精度覆盖; 缺少 CUDA/数值验证覆盖 |
...
| 14 | `Expand` | yes | yes | yes | `expand_forward` | `expand_forward` | no | no | 已 pytest 语义验证 | 含 Python 调度或 fallback; 已有 ONNX reference pytest 语义/混合精度覆盖; 缺少 CUDA/数值验证覆盖 |
| 15 | `Flatten` | yes | yes | yes | `flatten_forward` | `flatten_forward` | no | no | 已 pytest 语义验证 | 含 Python 调度或 fallback; 已有 ONNX reference pytest 语义/混合精度覆盖; 缺少 CUDA/数值验证覆盖 |
| 16 | `StringNormalizer` | yes | yes | yes | none | Python orchestration | no | no | 已实现未数值验证 | Python 调度/元数据类，不要求 C 数值后端; 缺少 CUDA/数值验证覆盖 |
| 17 | `Greater` | yes | yes | yes | `greater_forward` | `greater_forward` | yes | yes | 已数值验证 | - |
| 18 | `SequenceConstruct` | yes | yes | yes | none | Python orchestration | no | no | 已实现未数值验证 | Python 调度/元数据类，不要求 C 数值后端; 缺少 CUDA/数值验证覆盖 |
| 19 | `MelWeightMatrix` | yes | yes | yes | `mel_weight_matrix_forward` | `mel_weight_matrix_forward` | no | no | 已实现未数值验证 | 含 Python 调度或 fallback; 缺少 CUDA/数值验证覆盖 |
...
| 40 | `ThresholdedRelu` | yes | yes | yes | `thresholded_relu_forward` | `thresholded_relu_forward` | no | no | 已 pytest 语义验证 | 已有 ONNX reference pytest 语义/混合精度覆盖; 缺少 CUDA/数值验证覆盖 |
...
| 49 | `RandomNormal` | yes | yes | yes | `random_normal_forward` | `random_normal_forward` | no | no | 已实现未数值验证 | 缺少 CUDA/数值验证覆盖 |
| 50 | `Det` | yes | yes | yes | `det_forward` | `det_forward` | no | no | 已实现未数值验证 | 含 Python 调度或 fallback; 缺少 CUDA/数值验证覆盖 |
| 51 | `MaxPool` | yes | yes | yes | `max_pool_forward` | `max_pool_forward` | yes | yes | 已数值验证 | 含 Python 调度或 fallback |
| 52 | `ConvTranspose` | yes | yes | yes | `conv_transpose2d_forward` | `conv_transpose2d_forward` | yes | yes | 已数值验证 | 含 Python 调度或 fallback |
| 53 | `ConcatFromSequence` | yes | yes | yes | none | Python orchestration | no | no | 已实现未数值验证 | Python 调度/元数据类，不要求 C 数值后端; 缺少 CUDA/数值验证覆盖 |
...
| 64 | `Shrink` | yes | yes | yes | `shrink_forward` | `shrink_forward` | no | no | 已 pytest 语义验证 | 已有 ONNX reference pytest 语义/混合精度覆盖; 缺少 CUDA/数值验证覆盖 |
...
| 94 | `Asin` | yes | yes | yes | `asin_forward` | `asin_forward` | no | no | 已 pytest 语义验证 | 已有 ONNX reference pytest 语义/混合精度覆盖; 缺少 CUDA/数值验证覆盖 |
...
| 96 | `OptionalGetElement` | yes | yes | yes | none | Python orchestration | no | no | 已实现未数值验证 | Python 调度/元数据类，不要求 C 数值后端; 缺少 CUDA/数值验证覆盖 |
...
| 138 | `DynamicQuantizeLinear` | yes | yes | yes | `dynamic_quantize_linear_forward` | `dynamic_quantize_linear_forward` | no | no | 已实现未数值验证 | 缺少 CUDA/数值验证覆盖 |
| 139 | `HannWindow` | yes | yes | yes | `hann_window_forward` | `hann_window_forward` | no | no | 已实现未数值验证 | 缺少 CUDA/数值验证覆盖 |
...
| 143 | `BitwiseAnd` | yes | yes | yes | `bitwise_and_forward` | `bitwise_and_forward` | no | no | 已实现未数值验证 | 缺少 CUDA/数值验证覆盖 |
| 144 | `Scan` | yes | yes | yes | none | Python orchestration | no | no | 已实现未数值验证 | Python 调度/元数据类，不要求 C 数值后端; 缺少 CUDA/数值验证覆盖 |
| 145 | `HammingWindow` | yes | yes | yes | `hamming_window_forward` | `hamming_window_forward` | no | no | 已实现未数值验证 | 缺少 CUDA/数值验证覆盖 |
...
| 148 | `BitwiseOr` | yes | yes | yes | `bitwise_or_forward` | `bitwise_or_forward` | no | no | 已实现未数值验证 | 缺少 CUDA/数值验证覆盖 |
...
| 150 | `BlackmanWindow` | yes | yes | yes | `blackman_window_forward` | `blackman_window_forward` | no | no | 已实现未数值验证 | 缺少 CUDA/数值验证覆盖 |
