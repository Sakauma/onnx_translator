...
- CUDA verifier：134 个。
- active numerical plan 覆盖：134 个唯一算子名称，490 条默认计划。
- active numerical plan 混合精度覆盖：350 条默认计划。
...
- 默认数值门禁当前覆盖 134 个唯一算子；尚有 0 个已实现算子未进入 active numerical plan。
...
| 135 | `BatchNormalization` | yes | yes | yes | `batch_norm_forward` | `batch_norm_forward` | yes | yes | 已数值验证 | 含 Python 调度或 fallback; 已有 ONNX reference pytest 语义/混合精度覆盖 |
