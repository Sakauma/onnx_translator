...
- CUDA verifier：132 个。
- active numerical plan 覆盖：132 个唯一算子名称，484 条默认计划。
- active numerical plan 混合精度覆盖：346 条默认计划。
...
- 默认数值门禁当前覆盖 132 个唯一算子；尚有 0 个已实现算子未进入 active numerical plan。
...
| 29 | `LRN` | yes | yes | yes | `lrn_forward` | `lrn_forward` | yes | yes | 已数值验证 | 含 Python 调度或 fallback; 已有独立 pytest 深度语义/混合精度覆盖 |
