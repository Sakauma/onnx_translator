// verify_add.cu - CUDA验证程序，用于测试ADD操作的正确性
#include <stdio.h>
#include <stdlib.h>
#include <cuda_runtime.h>

/**
 * CUDA核函数：计算float32类型数据的逐元素加法
 *
 ...
 */
__global__ void add_kernel_float32(const float* input1, const float* input2, float* output, size_t size) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < size) {
        // ADD的核心逻辑: output = input1 + input2
...
int main(int argc, char** argv) {
    ...
    add_kernel_float32<<<blocks, threads>>>(d_input1, d_input2, d_output, num_elements);
